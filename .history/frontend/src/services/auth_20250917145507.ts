...
class AuthService {
  ...
  async login(credentials: LoginRequest): Promise<LoginResponse> {
    try {
      ...
      console.log('Token to save:', response.token?.access_token)
      
      // Verificar se o token existe antes de salvar
      if (response.token?.access_token) {
        // Salvar token diretamente no localStorage
        localStorage.setItem('auth_token', response.token.access_token)
        console.log('Token saved directly to localStorage:', response.token.access_token)
        console.log('Verifying token in localStorage:', localStorage.getItem('auth_token'))
        
        // Também atualizar o apiService
      ...
      if (response.user) {
        ...
        console.log('User saved, verifying...', localStorage.getItem('current_user'))
      } else {
